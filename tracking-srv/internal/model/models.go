...
type TrackingCacheLog struct {
	...
	ServiceClass   string    `gorm:"column:service_class;type:varchar(100)"`
	SyncedAt       int64     `gorm:"column:synced_at"`
	CreatedAt      time.Time `gorm:"column:created_at;autoCreateTime"`      // 这个保留（缓冲表不需要独立字段）
}
...
// TrackingReplace 数据变更备份表 model
type TrackingReplace struct {
	ID             int64     `gorm:"column:id;primaryKey;autoIncrement"`
	TrackingNumber string    `gorm:"column:tracking_number;type:varchar(100);not null;index"`
	OldDetail      string    `gorm:"column:old_detail;type:jsonb"`          // JSONB 类型
	NewDetail      string    `gorm:"column:new_detail;type:jsonb"`          // JSONB 类型
	Status         int32     `gorm:"column:status;default:0"`
	ServiceClass   string    `gorm:"column:service_class;type:varchar(100)"`
	SyncedAt       int64     `gorm:"column:synced_at"`
	CreatedAt      time.Time `gorm:"column:created_at;autoCreateTime"`
...
// TableName 指定表名
func (TrackingReplace) TableName() string {
