...
import (
	...
	"ns-tracking-go/service/tracking/api/internal/formatter"
	"ns-tracking-go/service/tracking/api/internal/normalize"
...
func (l *TisPushLogic) TisPush(req *types.TisPushRequest) (*types.Response, error) {
	// 1. 转换 TIS Push 格式 → WebhookRequest（OMS 格式）
	webhookReq := l.convertTisToWebhook(req.Data)

	// 2. 提取时间戳（用于 tracking_logs 同步）
	receivedAt, deliveredAt, trackedAt := l.extractTimestamps(req.Data.TrackEvents)

	// 3. 格式化数据（复用 YunExpressFormatter）
	raw := l.convertWebhookRequestToMap(webhookReq)
	f := formatter.NewYunExpressFormatter(raw)
	formatted := f.Format()
	...
	grpcReq := &tracking.UpsertRequest{
		...
		ReceivedAt:     receivedAt,    // 揽收时间
		DeliveredAt:    deliveredAt,   // 签收时间
		TrackedAt:      trackedAt,     // 最后轨迹时间
	}
